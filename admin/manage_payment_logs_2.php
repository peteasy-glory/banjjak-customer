...
<script>
	...
	var artist_list = []; // 펫샵
	var pay_type = {POS: 0, CARD: 0, BANK: 0, OFFLINE: 0}; // 결제수단(매장접수, 카드, 현금, 매장결제)
	...
	$manage_payment_log.on("click", ".get_search_btn", function(){
		...
		artist_list = []; // 펫샵
		pay_type = {POS: 0, CARD: 0, BANK: 0, OFFLINE: 0}; // 결제수단(매장접수, 카드, 현금, 매장결제)
	...
	function get_beauty_payment_log(post_data){
		return new Promise(function(resolve, reject) {
			...
			$.ajax({
				...
				pay_type_summary += (pay_type['BANK'] > 0)? '[계좌이체 '+pay_type['BANK']+'건]' : '';
