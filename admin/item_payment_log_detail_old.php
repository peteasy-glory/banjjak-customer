...
function get_item_payment_log_jbook(order_num, jbOrdNo){
	var _jbOrdNo = (jbOrdNo && jbOrdNo != "")? jbOrdNo : "";
				...
				if(json.code == "000000"){
					...
					$.each(json.data, function(i, v){
						...
						if(v.receipt_id && v.receipt_id.indexOf('INIpayTest') != -1){
							var html2 = '';
							html2 += '<div class="cancel_box">';
							html2 += '	테스트 주문건!! (정글북 상품일 경우 따로 취소요청 필요!!) ';
							html2 += '</div>';

							$("#item_order_detail .order_box_title").after(html2);
							$("#item_order_detail .item_payment").html('<div style="background-color: #fcc; color: #f00; text-align: center; padding: 10px;">테스트 거래 상태변경 불가</div>');
						}
						...
						if(v.is_cancel == '2'){
							...
							html2 += '<div class="cancel_box">';
							html2 += '	고객 취소사유 : '+cancel_reason[v.cancel_result.split('|')[0]]+' ('+v.cancel_dt+')';
							html2 += (v.cancel_result.split('|')[0] == 'etc')? '<span>'+v.cancel_result.split('|')[1]+'</span>' : '';
							if(v.cancel_result2.split('|')[1] == "2"){
								html2 += '	<br/>(환불계좌 : '+banks[fillZero(3, v.cancel_result2.split('|')[3])]+' '+v.cancel_result2.split('|')[2]+' '+v.cancel_result2.split('|')[4].format()+'원)';
							}
							html2 += '</div>';
						...
						if(v.is_return == '2'){
							...
							html2 += '<div class="return_box">';
							html2 += '	고객 반품사유 : '+return_reason[v.return_result.split('|')[0]]+' ('+v.return_dt+')';
							html2 += (v.return_result.split('|')[0] == 'etc')? '<span>'+v.return_result.split('|')[1]+'</span>' : '';
							if(v.return_result2.split('|')[1] == "2"){
								html2 += '	<br/>(환불계좌 : '+banks[fillZero(3, v.return_result2.split('|')[3])]+' '+v.return_result2.split('|')[2]+' '+v.return_result2.split('|')[4].format()+'원)';
							}
							html2 += '</div>';
	...
	return new Promise(function(resolve, reject) {
		...
		$.each(pay_data, function(i, v){ // thumbnail - product_no
			if(v.is_supply == "1" && v.supplier == "정글북"){
				$("#item_order_detail .item_option_wrap .item_cart_wrap[data-id='"+ip_log_seq+"'][data-item_no='"+target+"'] .item_detail").attr('data-jbookNo', v.goodsNo);
				get_jbook_list("goods", v.goodsNo, ip_log_seq, target);
			}
			var post_data = {};
			if(v.seq && v.seq != ""){
...
function get_jbook_list(menu, option, ip_log_seq, target){
			...
			if(data.code == "000000"){
				...
				if(data.data && parseInt(data.data.total) > 0){
					$.each(data.data.data, function(i, v){
						result_txt = (v.runout == "1")? '<span style="color:#f00;">품절</span>' : v.goodsStock+'개';
					});
				}
				$("#item_order_detail .item_option_wrap .item_cart_wrap[data-id='"+ip_log_seq+"'][data-item_no='"+target+"'] .item_detail[data-jbookNo='"+option+"']").append('<div>현재수량 : '+result_txt+'</div>');
			}else{
				alert(data.data+"("+data.code+")");
				console.log(data.code);
			}
...
$(document).on("click", "#item_order_detail .jbOrderCallBtn", function(){ // 정글북 주문처리
	$.MessageBox({
		buttonDone: "확인",
		buttonFail: "취소",
		message: "정글북 주문접수 하시겠습니까?"
	}).done(function(){
		get_jbook_order_call(no); // no	
	});
});
	...
	$.ajax({
		...
		success: function(data) {
			if(data.code == "000000"){
				...
				$("#item_order_detail .jbook_list").remove();
				get_item_payment_log_jbook(option, data.data);
			}else{
				alert(data.data+"("+data.code+")");
				console.log(data.code);
			}
