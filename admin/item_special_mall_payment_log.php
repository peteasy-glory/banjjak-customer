...
<style>
	...
	#admin_item_payment_log .order_table tr th { padding: 5px; background-color: #ddd; }
...
$(document).on("change", ".search_pay_status, .search_item_list", function(){
	location.href = "item_special_mall_payment_log.php?ps="+$(".search_pay_status").children("option:selected").val()+"&il="+$(".search_item_list").children("option:selected").val()+"&cn="+$(".search_customer_name").val();
});
...
$(document).on("click", ".search_reset_btn", function(){
	location.href = "item_special_mall_payment_log.php";
});
...
$(document).on("click", "#admin_item_payment_log .go_item_payment_detail_btn", function(){
	...
	location.href = '<?=$admin_directory?>/item_special_mall_payment_log_detail.php?no='+order_num+'&backurl='+encodeURIComponent(window.location.pathname+"?ps="+search_pay_status+"&il="+search_item_list);
});
...
*/
	...
	function get_file_list(target, product_no, img_list, goodsRepImage){
		return new Promise(function(resolve, reject) {
			//console.log(img_list != "", goodsRepImage);
			// img_loading
