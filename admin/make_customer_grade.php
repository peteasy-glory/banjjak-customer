...
<div class="bjj_header">
	...
	<div class="top_title">예약내역있는 고객 등급 설정</div>
</div>
...
<?php
	...
	if ($seq == 1) {
		...
		$sql = "
			SELECT a.customer_id, b.artist_id, b.idx FROM (
				SELECT * FROM tb_payment_log WHERE STATUS != 'POS' AND is_cancel = 0 AND is_no_show = 0 
				GROUP BY customer_id, artist_id
			) a
			LEFT JOIN tb_grade_of_shop b ON a.artist_id = b.artist_id
			WHERE b.grade_ord = 1 
		...
		while ($result_datas = mysql_fetch_object($result)) {
			...
			$idx = ($result_datas->idx)? $result_datas->idx : "";
			$artist_id = ($result_datas->artist_id)? $result_datas->artist_id : "";
			



			//if($customer_id == '-db-3@hanmail.net'){
				$select_sql = "
				...
				if($select_count>0){
					...
					$insert_sql = "
						INSERT INTO `tb_grade_of_customer` (`grade_idx`, `customer_id`, `is_delete`) VALUES (".$idx.", '".$customer_id."', 0);
					";
