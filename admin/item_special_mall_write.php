...
<style>
	...
	#item_group_option .item_group_option_detail_list_wrap .set_write_item_group_option_detail_box ul li .img_list .upload_file_wrap .set_delete_option_file_btn { position: absolute; right: 0px; top: 0px; width: 20px; height: 20px; line-height: 20px; text-align: center; background-color: rgba(255,255,255,0.8); border-radius: 10px; padding-top: 2px; }
...
<div class="bjj_top_menu">
    ...
    <div class="bjj_top_title"><p>전문몰 상품 내역</p></div>
		...
		<form id="item_form">
			...
			<ul>
				...
				<li>
					...
					<div class="value">
						<span>
							<input type="checkbox" id="is_use_point" name="is_use_point" value="1" />
        ...
        dataType: 'json',
			...
			if(data.result.code == "000000"){
				...
				$.each(data.result.data, function (index, file) {
					...
					if(file.target == "tb_item_list.product_img"){
						img_list.push(file.f_seq);
						img_list = img_list.filter(function(v){return v!==''}); // remove empty data
						$("#item_write #item_form input[name='product_img']").val(img_list.join(","));
						get_file_list(img_list);
						upload_flag = "item_list";
					}else if(file.target == "tb_item_group_option_detail.image"){
						option_img_list.push(file.f_seq);
						option_img_list = option_img_list.filter(function(v){return v!==''}); // remove empty data
						$("#item_group_option .set_write_item_group_option_detail_box input[name='image']").val(option_img_list.join(","));
		...
		$.ajax({
			...
			success: function(data) {
				if(data.code == "000000"){
					var html = '';
					$.each(data.data, function(i, v){			
						html += '<div>'+v.cate_name+'<span class="del_category" data-id="'+v.ismc_seq+'"><i class="fas fa-times"></i></span></div>';
					});
...
$(document).on("click", "#category_wrap .selected_cate_btn", function(){
	...
	var ic_seq_list = $("#category_wrap .ic_seq_list").val();
	if(ic_seq_list != ""){
		if(ic_seq != ""){
			ic_seq = ic_seq.split(",");
			var chk_flag = 0;
			$.each(ic_seq, function(i, v){ //duplicate chk
				if(v == ic_seq_list){
					chk_flag = 1;
				}
			});
			if(chk_flag == 0){
				ic_seq.push(ic_seq_list);
				$("#item_write input[name='ic_seq']").val(ic_seq);
			}
		}else{
			$("#item_write input[name='ic_seq']").val(ic_seq_list);
		}

		selected_category_list();
		$("#category_wrap").dialog("close");	
		$("#category_wrap .ic_seq_list").val("");
	}
});
...
$item_group_option.on("click", ".set_delete_option_file_btn", function(){
	var f_seq = $(this).data("seq");
	$.MessageBox({
		buttonFail: "아니오",
		buttonDone: "예",
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(data.code == "000000"){
						...
						if(data.data && data.data.length > 0){
							$.each(data.data, function(i, v){
								option_img_list = (v.image && v.image != "")? ((v.image.indexOf(',') != false)? v.image.split(',') : [v.image]) : [];
								get_file_list_option(option_img_list);
							});
						}
			...
			success: function(data) {
				if(data.code == "000000"){
					...
					$.each(data.data, function(i, v){
						...
						html += '		<img src="'+v.file_path+'" alt="'+v.file_name+'" title="'+v.file_name+'" />';
						html += '	</a>';
						html += '	<div class="set_delete_option_file_btn" data-seq="'+v.f_seq+'"><i class="fas fa-times"></i></div>';
						html += '</div>';
						idx++;
					});
...
function set_delete_option_file(f_seq){
	$.ajax({
		url: '<?=$mainpage_directory?>/fileupload_ajax.php',
		data: {
			mode: "set_delete_file",
