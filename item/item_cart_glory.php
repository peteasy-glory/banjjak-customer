...
<script>
	...
	var is_free_shipping_limit = ["", 100000, 30000, 50000]; // '', 업체배송, 직배송
	var is_free_shipping_total = ["", 0, 0, 0]; // '', 업체배송, 직배송
	var is_free_shipping_cnt = ["", 0, 0, 0]; // '', 업체배송, 직배송
	var is_free_shipping_price = ["", 0, 0, 0]; // '', 업체배송, 직배송
		...
		return new Promise(function(resolve, reject) {
			...
			if(item_update_flag == item_cart_cnt){ // each end flag
				...
				if(is_free_shipping_price[1] == 0 && is_free_shipping_price[2] == 0 && is_free_shipping_price[3] == 0){
					...
					$item_cart.find('.in_product .shipping_cost_dc .dc_info').html((is_free_shipping_limit[3]).format()+'원 추가하면 무료배송');
					$item_cart.find('.in_product .shipping_cost_dc .dc_progressbar .ui-progressbar-value').animate({width: '0%'}, 100);
				}else{
					...
					if(is_free_shipping_price[3] >= is_free_shipping_limit[3]){ // 5만원 이상 구매시
						_shipping_price[3] = 0;

						$item_cart.find('.in_product .shipping_cost_dc .dc_info').html('무료배송');
						$item_cart.find('.in_product .shipping_cost_dc .dc_progressbar .ui-progressbar-value').animate({width: '100%'}, 100);
					}else{
						if(is_free_shipping_total[3] == is_free_shipping_cnt[3]){ // 전문상품 전부 무료일 경우 무료
							_shipping_price[3] = 0;
							...
							$item_cart.find('.in_product .shipping_cost_dc .dc_info').html((is_free_shipping_limit[3] - is_free_shipping_price[3]).format()+'원 추가하면 무료배송');
							$item_cart.find('.in_product .shipping_cost_dc .dc_progressbar .ui-progressbar-value').animate({width: Math.round(is_free_shipping_price[3] / is_free_shipping_limit[3] * 100)+'%'}, 100);
						}
					}
				...
				$item_cart.find('.in_product .product_price_wrap .product_price_txt').html((is_free_shipping_price[3]).format());
				...
				$item_cart.find('.in_product .product_price_wrap .shipping_cost_txt').html((_shipping_price[3]).format());
					...
					if(data.code == "000000"){
						...
						if(data.data && data.data.list && data.data.list.length > 0){
							$.each(data.data.list, function(i, v){
								...
								html += '			<div class="item">';
								html += '				<div class="item_data" data-id="'+v.il_seq+'">';
								if(v.is_soldout == "2"){
