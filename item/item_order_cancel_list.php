...
<script>
					...
					if(data.code == "000000"){
						...
						if(data.data && data.data.length > 0){
							$.each(data.data, function(i, v){
								...
								if(v.pay_status == "1" || v.pay_status == "2"){ // 상품 준비/배송 준비된 상품만 반품 가능
							...
							$("#item_order_cancel .item_payment_log_wrap .item_cart").append(html);

							$.each(data.data, function(i, v){
								$("#item_order_cancel .item_payment_log_wrap .item_cart_wrap[data-id='"+v.iplp_seq+"'][data-item_no='"+i+"'] .item_cart_data").attr('data-id', v.product_no);
	...
	function get_item_option(pay_data){
	...
	function get_item(ip_log_seq, target, pay_data, product_name, pay_status, shipping_invoice, shipping_company){
		return new Promise(function(resolve, reject) {
			...
			$.each(pay_data, function(i, v){
				...
				html += '			<label for="chk_ag'+ip_log_seq+''+target+''+i+'"><span></span>취소</label>';
				html += '		</div>';
				html += '		<div class="item_name">'+item_name+'</div>';
