...
<div id="fixed-menu">
	<div class="fixed-menu-wrap">
		...
		<div class="center_menu">반짝 설 선물세트
		</div>
...
<div id="event_6">
	<div class="banner">
		<img src="../images/view_event6_sul_banner_2.jpg" />
	</div>
...
<script>
	var $event = $("#event");
	var event_item_list = ['PE-SET-A01', 'PE-SET-A02', 'PE-SET-A03', 'PE-SET-A04', 'PE-SET-A05'];
	var flag = 0;
	var cnt = 10;
	var lastScrollTop = 0;
	var timer = null;
	...
	$event.on("click", ".item_link_btn", function(){
		console.log(localStorage.getItem('windowscrolltop_event6'));
		localStorage.setItem('windowscrolltop_event6',$(window).scrollTop()); // scroll position
	...
	$event.on("click", "#bj_item_cart", function(){
		history.replaceState('', '', window.location.pathname);
		location.href = "<?=$mainpage_directory ?>/item_cart.php?backurl="+encodeURIComponent(window.location.pathname);
	});
	...
	function get_event_item_list_html(){
		return new Promise(function(resolve, reject) {
			...
			html += '	<div class="title"><span>반짝 설 선물세트</span></div>';
	...
	function footer_html(){
		return new Promise(function(resolve, reject) {
			...
			*/
			...
			console.log(localStorage.getItem('windowscrolltop_event6'));
			if(localStorage.getItem('windowscrolltop_event6') > 0){
				$('html, body').animate({
					scrollTop: localStorage.getItem('windowscrolltop_event6')
				}, 500, function(){
					localStorage.setItem('windowscrolltop_event6', 0); // scroll position					
				});
			}
		});
	}
