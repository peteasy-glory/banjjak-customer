<?php header('Location: https://www.gopet.kr/pet/mainpage/index.php'); exit; // 2021-02-05 ulmo 새 페이지로 이동 ?>
<?php
							...
							<td colspan="2">
								<table width="100%">
									...
									<tbody>
										...
										<tr class="use_point event_first_payment_point">
											<td colspan="2"></td>
										</tr>
										<tr class="use_point">
...
<form id="INIpay_form" method="POST" action="<?=$INI_url['card']; ?>" accept-charset="euc-kr" onsubmit="emulAcceptCharset(this)" style="border: 1px solid #fcc;margin-bottom:10px;">
...
<form id="SendPayForm_id" name="" method="POST" style="border: 1px solid #ccf;">
...
var is_free_shipping_limit = ["", 100000, 30000]; // '', 업체배송, 직배송
var is_free_shipping_total = ["", 0, 0]; // '', 업체배송, 직배송
var is_free_shipping_cnt = ["", 0, 0]; // '', 업체배송, 직배송
var is_free_shipping_price = ["", 0, 0]; // '', 업체배송, 직배송
				...
				if(data.code == "000000"){
					...
					$.each(data.data.list, function(i, v){
						...
						$.each(parse_cart_data, function(i2, v2){
							...
							if(v.is_supply == "1"){ // 외주업체(정글북)
								orderItem.push({goodsNo: v.goodsNo, ea: v2.amount}); // 정글북 데이터 입력
							}
						});
					...
					$.each(data.data.list, function(i, v){
						...
						if(v.is_use_point == "1"){
							$(".use_point").css("display", "table-row");
						}
					});
...
function price_calculator(){
	return new Promise(function(resolve, reject) {
		...
		var _shipping_price = ['', 0, 0]; // 직배송 + 업체배송
		var _total_price = parseInt(is_free_shipping_price[1]) + parseInt(is_free_shipping_price[2]);
		
		if(is_free_shipping_price[1] == 0 && is_free_shipping_price[2] == 0){
			_shipping_price = ['', 0, 0];
		}else{
		...
		$("#item_order_sheet input[name='shipping_price']").val((_shipping_price[1] + _shipping_price[2]));
		$("#item_order_sheet input[name='total_price']").val(_total_price + (_shipping_price[1] + _shipping_price[2]));
		$("#item_order_sheet .product_price").text(_total_price.format());
		$("#item_order_sheet .shipping_price").text((_shipping_price[1] + _shipping_price[2]).format());
		$("#item_order_sheet .total_price").text((_total_price + (_shipping_price[1] + _shipping_price[2])).format());
		$("#item_order_sheet .order_data_wrap .total_price_txt").text((_total_price + (_shipping_price[1] + _shipping_price[2])).format());

		resolve([_total_price, (_shipping_price[1] + _shipping_price[2])]);
	});
}
...
function change_payment(type){
	var reserve = "vbank_receipt=N&bank_receipt=N&cp_yn=N&noeasypay=Y";
	var reserve2 = "Card";
	var reserve3 = "HPP(1):no_receipt:va_receipt:vbanknoreg(0):below1000:noeasypay";
...
function set_update_item_payment_log_price_zero(){
	...
	$.ajax({
		...
		success: function(json) {
			if(json.code == "000000"){
				console.log(json.data);
				location.href = "<?=$mainpage_directory ?>/item_payment_result.php?no="+no;
			}else{
				alert(json.data+"("+json.code+")");
			}
		},
