...
$customer_result = mysqli_query($connection, $sql);
$old_phone = mysqli_fetch_object($customer_result);
...
//global $connection;
  ...
  if($result){
      //발신자 번호 수정
      $sql = "update tb_sent_cell_id set cellphone = '".$No."' where cellphone = '".$old_phone->cellphone."';";
      $result = mysqli_query($connection, $sql);
      $sql = "update tb_payment_log set cellphone = '".$No."' where cellphone = '".$old_phone->cellphone."';";
      $result = mysqli_query($connection, $sql);
      if($result){
